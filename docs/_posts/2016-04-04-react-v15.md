...
We also made a bugfix to `React.cloneElement(element, props)` that is technically a breaking change. If some of the passed `props` were `undefined`, `cloneElement()` in React 0.14 returned an element with `undefined` values for those props. In React 15, `cloneElement()` is consistent with `createElement()`, and any `undefined` props are resolved to the corresponding component’s `defaultProps`. In a codebase with more than 20,000 React components, we only a single place affected by this so we feel comfortable releasing it without keeping the old behavior for another release cycle. The new behavior should usually be more correct. ([@truongduy134](https://github.com/facebook/react/pull/5997))

There is also a minor breaking change that won’t affect applications but may affect some third-party tools. We are working on [revamping `ReactPerf` instrumentation](https://github.com/facebook/react/pull/6046) to provide more detailed information. We plan to release it during the 15.x cycle so the internal performance measurement format will change. This is why the return value of `ReactPerf.getLastMeasurements()` is now considered an opaque data structure. ([@gaearon](https://github.com/facebook/react/pull/6286))
...
- React 0.14 uses DOM nodes as `refs` to the DOM elements. Accessing React-specific properties on them (e.g. `this.refs.div.props`) was deprecated in React 0.14 and is not supported in React 15. ([@jimfb](https://github.com/facebook/react/pull/5495))
...
- React DOM now warns if you specify `onFocusIn` or `onFocusOut` handlers which are normalized to `onFocus` in React. ([@jontewks](https://github.com/facebook/react/pull/6296))
...
- `componentWillReceiveProps()` is now consistently called when `context` changes. ([@milesj](https://github.com/facebook/react/pull/5787))
- `React.cloneElement()` attempts to preserve the existing `key` when used inside `React.Children.map()`. ([@ianobermiller](https://github.com/facebook/react/pull/5892))
...
- Add-Ons: ReactPerf no longer instruments adding and removing an event listener because they don’t really touch the DOM due to event delegation. ([@antoaravinth](https://github.com/facebook/react/pull/5209))
...
- Shallow renderer now returns the rendered element. ([@simonewebdesign](https://github.com/facebook/react/pull/5411))
- React removes the dependency on ES5 shams for `Object.create` and `Object.freeze` in older environments. ([@dgreensp](https://github.com/facebook/react/pull/4959))
...
- React improves performance for `createClass()` on complex specs. ([@spicyj](https://github.com/facebook/react/pull/5550))
